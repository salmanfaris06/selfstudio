...
import { assignGPhoto2Camera, type CameraAssignmentRequest } from "@/lib/api/client";
import { stationReadinessQueryKey } from "./use-station-readiness-query";
import { stationsQueryKey } from "./use-stations-query";

export function useCameraAssignmentMutation(stationId: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (request: CameraAssignmentRequest) => assignGPhoto2Camera(stationId, request),
    onSuccess: () => {
