...
if (-not (Test-Path (Join-Path $WebPath "node_modules"))) {
  throw "Missing apps/web/node_modules. Run scripts/build.ps1 or run npm install in apps/web first."
}

if (-not (Test-Path (Join-Path $WebPath ".next"))) {
  throw "Missing apps/web/.next production build. Run scripts/build.ps1 before scripts/start.ps1."
}

Write-Host "== Selfstudio Local Start =="
Write-Host "Agent health: http://localhost:$AgentPort/health"
Write-Host "Dashboard local: http://localhost:$WebPort"
if ($LanIp) { if ($AgentHost -eq "127.0.0.1") {
    Write-Host "Dashboard LAN: unavailable because agent is bound to 127.0.0.1"
  } else {
    Write-Host "Dashboard LAN: http://$LanIp`:$WebPort"
  } } else { Write-Host "Dashboard LAN: detection unavailable" }
